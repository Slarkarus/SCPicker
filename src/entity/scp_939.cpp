        ...
        for (int try_pos_x = 0; try_pos_x < field_width; ++try_pos_x)
        {
            for (int try_pos_y = 0; try_pos_y < field_height; ++try_pos_y)
            {
                if (!used[try_pos_y][try_pos_x] &&
                    ...
                    (closest_unused.first == -1 || dp[try_pos_y][try_pos_x] < dp[closest_unused.second][closest_unused.first]))
                {
        ...
        for (int y_shift = -1; y_shift <= 1; ++y_shift)
        {
            for (int x_shift = -1; x_shift <= 1; ++x_shift)
            {
                if (abs(x_shift) + abs(y_shift) == 1)
                {
                    ...
                    if (neighbour.first < 0 || neighbour.first >= field_width || neighbour.second < 0 || neighbour.second >= field_height)
                    {
                        continue;
                    }
...
std::vector<std::pair<double, double>> get_scp_939_path(std::pair<double, double> a, std::pair<double, double> b, Gamedata *gamedata, double SPEED)
{
    ...
    for (int i = 0; i + 1 < center_path.size(); ++i)
    {
        ...
        int num_iterations = (1.0 / SPEED);
        if (first_point.first != second_point.first)
        {
            double dx = second_point.first - first_point.first;
            for (int i = 0; i < num_iterations; ++i)
            {
                true_path.push_back({first_point.first + dx * SPEED * i, first_point.second});
            }
        }
        else
        {
            ...
            for (int i = 0; i < num_iterations; ++i)
            {
                true_path.push_back({first_point.first, first_point.second + dy * SPEED * i});
            }
        }
    }
...
{
    scp->next_path_ = get_scp_939_path(scp->get_pos(), gamedata->get_player()->get_pos(), gamedata, SCP_939_SPEED_PLAYER);
}
...
void set_random_path(ent::SCP_939 *scp, Gamedata *gamedata)
{
    ...
    scp->next_path_ = get_scp_939_path(scp->get_pos(), {(double)x + 0.5, (double)y + 0.5}, gamedata, SCP_939_SPEED_RANDOM);
